...
(function () {
  ...
  function setupTextSplit() {
    document.querySelectorAll('[data-split]').forEach(el => {
      ...
      el.style.overflow = 'clip visible';
      el.style.clipPath = 'inset(-10% -10% -10% -10%)';

      if (mode === 'chars') {
        el.innerHTML = text.split('').map(c =>
          c === ' ' ? ' ' : `<span style="display:inline-block; padding-bottom:0.15em">${c}</span>`
        ).join('');
        ...
        el.innerHTML = text.split(' ').map(w =>
          `<span style="display:inline-block; margin-right:0.25em; padding-bottom:0.15em">${w}</span>`
        ).join('');
        gsap.from(el.querySelectorAll('span'), {
          y: '100%',
          opacity: 0,
